...
const etsyApi = {
  ...
  // Trigger sync for the default connected shop (no shopId needed)
  sync:          () => axiosInstance.post('/api/v1/customer/etsy/sync').then(r => r.data),
  ...
  featureListing: (listingId, rank) => axiosInstance.patch(`/api/v1/customer/etsy/listings/${listingId}/featured`, { featured_rank: rank }).then(r => r.data),
